...
export const Profiles = () => {
  ...
  useEffect(() => {
    axios
      .get("https://api.cityrunner.site:4000/mypage", {
      ...
      .then((data) => {
        ...
        if (data.data.data.oauth) {
          ...
          if (data.data.data.image && data.data.data.image !== "default") {
            const UserImg = `https://api.cityrunner.site:4000/images/${data.data.data.image}`;
            setImages(UserImg);
          }
        }
      })
  ...
  const fileSelected = async (event) => {
    ...
    const newImg = `https://api.cityrunner.site:4000/${result.imagePath}`;
    setImages(newImg);
  };
