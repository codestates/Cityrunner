...
module.exports= {
    getMycrew: async (req,res)=>{
        ...
        let chattingRoom = await models.chattingRoom.findAll({where:{memberId:2},
            order:[["id" ,"DESC"]],
            limit: 1,
            raw:true
        })
        
        if(chattingRoom.length === 0) return res.status(204).json({message:"한번도 참가하지 않았군요"})
        ...
        let postLog = await models.post.findOne({where:{id:chattingRoom[0].postId}})
        postLog = postLog.dataValues

        sendAccessToken(res,accessToken)
        
        if(userTodayData !== todayData)  return res.status(205).json({message:"아직 참가중인 방 없습니다"})

        return res.status(200).json({data :postLog , message:"참가중인 방입니다"})
